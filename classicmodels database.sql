...
select monthname(paymentdate) as payment_month,count(customerNumber) as num_payments from payments group by payment_month having num_payments > 20 order by num_payments desc;
use test1;
create table customers(
...
USE CLASSICmodels;
select country,count(ordernumber) as order_count from customers join orders on customers.customernumber = orders.customernumber group by ordernumber;
select count(ordernumber) from orders;
select country,ordernumber as order_count from customers join orders on customers.customernumber = orders.customernumber;
...
WHERE
    c.country IS NOT NULL  -- Ensure country is not NULL
GROUP BY
    c.country
ORDER BY
    order_count DESC
